...
public class RussianRouletteRe {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<bullet; i++) {
			...
			if(bulletPos[position]) {
				i--; //총알이 true이므로 1빼기.
				continue;
			} else { //bulletPos[position] = false 이면 중복되기 때문에 true로 바꿔줘야 함.
				bulletPos[position] = true;
			}
		} //총알 배치 끝.

		//실행(처음으로 게임을 시작할 사람)순서 정하기 //랜덤배정
		...
		while(true) {
			...
			if(bulletPos[realBulletPos]) { //총알 격발!
				...
				System.out.printf("\n빵!!!\n[%s] 사망...\n"
						, players[startIdx]);
				bullet--; //총알을 1개 소비 했기 때문에 
				playerNum--;//게임 참가자가 1명 죽었기 때문에
				bulletPos[realBulletPos] = false; // 총알이 소모되었으니 true에서 false로 변경.
				...
				String[] temp = new String[players.length-1]; //죽은사람을 제외한 새로운 참가자 배열 선언.(임시 배열)
				...
				if(players.length == 1) { //한명만 배열에 존재하므로 게임 종료.
					System.out.println("최종 생존자: " + players[0]);
					System.out.println("게임을 종료합니다.");
					break;
				} else if(bullet == 0) { //총알이 없다면 게임 종료.
			...
			realBulletPos++;// bulletPos의 6개의 값중 첫번째 값을 소모했기 때문에 1을 올려줌.
		}
